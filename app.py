...
<div class="chat-area">
    <div class="chat-header">
        <div class="top-bar">
            ...
            <button id="supportBtn" class="notify-btn" style="background:#3b82f6;" title="Поддержка">✉️</button>
            <button id="offlineBtn" class="notify-btn" style="background:#8b5cf6;" title="Офлайн пользователи">👥 Офлайн</button>
        </div>
...
<div id="offlineModal" class="modal"><div class="modal-content" style="max-width:400px;"><span class="close" id="closeOfflineModal">&times;</span><h3>👥 Офлайн пользователи</h3><div id="offlineUsersList"></div></div></div>
...
// Личные сообщения (полноэкранные)
function loadDMList(){
...
document.getElementById('offlineBtn').onclick=()=>{
    fetch('/get_offline_users').then(r=>r.json()).then(data=>{
        let container=document.getElementById('offlineUsersList');
        if(!data.length){container.innerHTML='<p>Нет офлайн пользователей</p>';}
        else{
            container.innerHTML=data.map(u=>`<div class="offline-user-item"><div class="user-avatar" style="width:32px;height:32px;">${u.avatar_base64?`<img src="${u.avatar_base64}" style="width:100%;height:100%;">`:'👤'}</div><div>${escape(u.name)} (${u.role==='owner'?'Владелец':(u.role==='admin'?'Админ':(u.role==='moderator'?'Модератор':'Пользователь'))})</div></div>`).join('');
        }
        document.getElementById('offlineModal').style.display='flex';
    });
};
document.getElementById('closeOfflineModal').onclick=()=>document.getElementById('offlineModal').style.display='none';
// Поддержка
document.getElementById('supportBtn').onclick=()=>window.location.href='mailto:vikogrod@gmail.com';
// Жалобы
...
socket.on('update_online_users',onlineList=>{/* можно обновить счётчик онлайн, но для офлайн он не нужен */});
// Отправка текста
...
window.onclick=e=>{
    ...
    if(e.target===document.getElementById('offlineModal'))document.getElementById('offlineModal').style.display='none';
...
def get_offline_users():
    ...
    for name, u in users.items():
        if not u.get('banned') and name not in online_users:
            offline.append({'name': name, 'role': u['role'], 'avatar_base64': u.get('avatar_base64')})
