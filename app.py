...
app = Flask(__name__)
app.secret_key = 'final-secret-key-2024'
...
# -------------------------- HTML ШАБЛОН (ОГРОМНЫЙ, НО ПОЛНЫЙ) --------------------------
# Весь HTML собран в одну переменную CHAT_HTML (он будет передан render_template_string)
CHAT_HTML = '''
<!DOCTYPE html>
<html>
...
<div class="chat-area">
    <div class="chat-header">
        <div class="header-buttons">
            ...
            <button class="dm-back-btn" id="dmBackBtn">← Назад</button>
        </div>
    ...
    <div class="input-area">
        ...
        <input type="file" id="fileInput" style="display:none" accept="image/*,application/pdf,text/plain,audio/*">
...
<div id="settingsModal" class="modal"><div class="modal-content"><span class="close" id="closeSettings">&times;</span><h3>⚙️ Настройки</h3><button id="themeBtn">🌙 Тёмная тема</button><button id="clearHistoryBtn">🗑️ Очистить историю</button><button id="exportChatBtn">📁 Экспорт чата</button><button id="changePasswordBtn">🔑 Сменить пароль</button><button id="notificationsSettingsBtn">🔔 Уведомления</button>{% if role in ["owner","admin"] %}<button id="viewReportsBtn">📋 Жалобы</button>{% endif %}</div></div>
...
function updateBadge(){let b=document.getElementById('notifyBadge');let c=notifications.length+friendRequests.length;if(c>0){b.textContent=c>99?'99+':c;b.style.display='flex';}else b.style.display='none';}
function updateNotifList(){
    let c=document.getElementById('notificationsList');
    if(notifications.length===0&&friendRequests.length===0){c.innerHTML='<p style="color:#6b7280;text-align:center;padding:16px">Нет уведомлений</p>';return;}
    let h='';
...
function loadRequests(){fetch('/get_friend_requests').then(r=>r.json()).then(data=>{friendRequests=data.requests||[];updateBadge();updateNotifList();});}
...
function addDM(name,text,time,isOwn){
    ...
    fetch('/user_avatar/'+encodeURIComponent(name)).then(r=>r.json()).then(avatar=>{
        if(avatar.avatar_base64)avatarHtml=`<img src="${avatar.avatar_base64}">`;else avatarHtml=avatar.avatar||'👤';
        div.innerHTML=`
...
function showUserProfile(name){
    fetch('/user_info/'+encodeURIComponent(name)).then(r=>r.json()).then(data=>{
        ...
        if(role==='owner'||role==='admin'){
            if(data.role==='admin')actions+=`<button onclick="unadminUser('${name}')" style="background:#f59e0b;color:#fff">🔻 Снять админку</button>`;
            else if(data.role!=='owner')actions+=`<button onclick="giveAdmin('${name}')" style="background:#10b981;color:#fff">⭐ Выдать админку</button>`;
            if(data.role==='moderator')actions+=`<button onclick="unmoderUser('${name}')" style="background:#f59e0b;color:#fff">🔻 Снять модератора</button>`;
            else if(data.role!=='owner'&&data.role!=='admin')actions+=`<button onclick="giveModer('${name}')" style="background:#f59e0b;color:#fff">⭐ Выдать модератора</button>`;
        }
        if(role==='owner'||role==='admin'){
...
window.reportUser=name=>{currentReportUser=name;let reason=prompt('Причина жалобы:');if(reason){fetch('/report_user',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({target:currentReportUser,reason:reason})}).then(r=>r.json()).then(data=>{showToast(data.message,data.success?'success':'error');if(data.success)addNotification('Жалоба отправлена',`На ${currentReportUser}`);});}};
...
window.addFriend=name=>{fetch('/add_friend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({friend:name})}).then(r=>r.json()).then(data=>{addSystem(data.message);showToast(data.message,'success');addNotification('Заявка',name);document.getElementById('userModal').style.display='none';loadRequests();});};
...
function deleteRoom(roomName){
    confirmDialog(`Удалить комнату "${roomName}"?`,()=>{
        fetch('/delete_room',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({room:roomName})}).then(r=>r.json()).then(data=>{
            ...
            if(data.success && currentRoom===roomName && !isDMmode){
                socket.emit('switch',{old:currentRoom,new:'Главная'});
                currentRoom='Главная';
                document.getElementById('roomName').innerText='Главная';
            }
...
window.onclick=e=>{
    ...
    if(e.target===document.getElementById('dmModal'))document.getElementById('dmModal').style.display='none';
};
...
def register():
    if request.method == 'POST':
        ...
        users[name] = {'password': hashlib.sha256(pwd.encode()).hexdigest(), 'role': 'user', 'avatar': '👤', 'avatar_base64': None, 'bio': '', 'banned': False, 'muted_until': None, 'online': False, 'user_id': generate_short_id(), 'id_change_count': 0, 'last_id_change': None, 'theme': 'light'}
...
def get_friend_requests():
    ...
    return jsonify({'requests': friends.get(session['username'], [])})
...
def user_info(name):
    ...
    return jsonify({'username':name,'bio':u.get('bio',''),'role_display':role_display,'role':u['role'],'avatar':u.get('avatar','👤'),'avatar_base64':u.get('avatar_base64'),'user_id':u.get('user_id',''),'friends_count':len(friends.get(name,[])),'is_friend':is_friend,'banned':u.get('banned',False),'muted':u.get('muted_until') and datetime.now()<datetime.fromisoformat(u['muted_until'])})

@app.route('/user_avatar/<name>')
def user_avatar(name):
    if name not in users: return jsonify({'avatar':'👤','avatar_base64':None})
    return jsonify({'avatar':users[name].get('avatar','👤'),'avatar_base64':users[name].get('avatar_base64')})
