...
def give_admin():
    user_data = get_user_by_session()
    if not user_data or users[user_data[0]]['role'] != 'owner':
...
def add_friend():
    ...
    save_users(users)
    socketio.emit('friend_request', {'from': username, 'to': target}, to=target)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: system-ui; background: linear-gradient(135deg, #667eea, #764ba2); height: 100vh; display: flex; transition: 0.3s; }
        body.dark { background: #1e1b4b; }
        .sidebar { width: 280px; background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); border-right: 1px solid rgba(0,0,0,0.1); display: flex; flex-direction: column; padding: 20px; overflow-y: auto; transition: 0.3s; }
        body.dark .sidebar { background: #1f2937; color: white; }
        .user-card { text-align: center; padding: 20px; background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 24px; color: white; margin-bottom: 20px; cursor: pointer; }
        .user-avatar { width: 64px; height: 64px; border-radius: 50%; background: rgba(255,255,255,0.2); display: flex; align-items: center; justify-content: center; margin: 0 auto 10px; font-size: 32px; overflow: hidden; }
        .user-avatar img { width: 100%; height: 100%; object-fit: cover; }
        .section-title { font-weight: 600; margin: 16px 0 8px 0; color: #374151; }
        body.dark .section-title { color: #9ca3af; }
        .room-item, .user-item { padding: 10px 12px; border-radius: 12px; cursor: pointer; margin-bottom: 4px; display: flex; align-items: center; gap: 8px; }
        .room-item:hover, .user-item:hover { background: rgba(0,0,0,0.05); }
        .room-item.active { background: #667eea; color: white; }
        .add-room { display: flex; gap: 8px; margin-top: 12px; }
        .add-room input { flex: 1; padding: 8px 12px; border: 1px solid #ddd; border-radius: 20px; outline: none; }
        .add-room button { background: #667eea; color: white; border: none; border-radius: 20px; padding: 8px 16px; cursor: pointer; }
        .chat-area { flex: 1; display: flex; flex-direction: column; background: white; transition: 0.3s; }
        body.dark .chat-area { background: #111827; }
        .chat-header { padding: 16px 24px; border-bottom: 1px solid #eee; background: white; display: flex; justify-content: space-between; }
        body.dark .chat-header { background: #1f2937; color: white; border-color: #374151; }
        .messages { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 12px; }
        .message { display: flex; gap: 10px; align-items: flex-start; }
        .message-own { justify-content: flex-end; }
        .message-avatar { width: 32px; height: 32px; border-radius: 50%; background: #667eea; display: flex; align-items: center; justify-content: center; font-size: 14px; color: white; overflow: hidden; }
        .message-content { background: #f3f4f6; padding: 8px 14px; border-radius: 18px; max-width: 60%; }
        body.dark .message-content { background: #374151; color: white; }
        .message-own .message-content { background: #667eea; color: white; }
        .message-name { font-size: 12px; font-weight: bold; margin-bottom: 4px; }
        .message-time { font-size: 10px; opacity: 0.6; margin-left: 8px; }
        .system-msg { text-align: center; font-size: 12px; color: #6b7280; padding: 4px; }
        .typing { padding: 8px 24px; font-size: 12px; color: #6b7280; font-style: italic; }
        .input-area { display: flex; gap: 12px; padding: 16px 24px; border-top: 1px solid #eee; background: white; }
        body.dark .input-area { background: #1f2937; border-color: #374151; }
        .input-area input { flex: 1; padding: 12px 18px; border: 1px solid #ddd; border-radius: 30px; outline: none; }
        body.dark .input-area input { background: #374151; color: white; border-color: #4b5563; }
        .input-area button { background: #667eea; border: none; border-radius: 50%; width: 46px; height: 46px; color: white; cursor: pointer; }
        .btn-settings, .btn-logout { margin-top: 10px; padding: 10px; border: none; border-radius: 20px; cursor: pointer; width: 100%; }
        .btn-settings { background: #e0e7ff; color: #4f46e5; }
        .btn-logout { background: #fee2e2; color: #dc2626; }
        body.dark .btn-settings { background: #374151; color: #818cf8; }
        body.dark .btn-logout { background: #374151; color: #f87171; }
        ...
        .modal-content { background: white; border-radius: 32px; padding: 32px; max-width: 400px; width: 90%; max-height: 80vh; overflow-y: auto; }
        body.dark .modal-content { background: #1f2937; color: white; }
        .modal-content input, .modal-content textarea { width: 100%; padding: 12px; margin: 10px 0; border: 1px solid #ddd; border-radius: 24px; }
        .modal-content button { background: #667eea; color: white; border: none; padding: 12px; border-radius: 24px; width: 100%; cursor: pointer; margin-top: 10px; }
...
<div id="profileModal" class="modal"><div class="modal-content">
    ...
    <label>О себе:</label>
    <textarea id="bioInput" rows="3" placeholder="Расскажите о себе...">{{ bio }}</textarea>
...
<div id="settingsModal" class="modal"><div class="modal-content">
    ...
    <h4 style="margin-top:20px;">📨 Заявки в друзья</h4>
...
<script>
    ...
    document.getElementById('avatarFile')?.addEventListener('change', function(e) {
        ...
        if(file && (file.type === 'image/jpeg' || file.type === 'image/png')) {
            ...
            reader.onload = (ev) => { tempAvatar = ev.target.result; };
    ...
    document.getElementById('saveProfileBtn')?.addEventListener('click', () => {
        fetch('/update_profile', {
            ...
            body:JSON.stringify({
                ...
                avatar_base64: tempAvatar,
                bio: document.getElementById('bioInput').value,
                new_name: document.getElementById('newNameInput').value,
                new_password: document.getElementById('newPasswordInput').value
            })
        }).then(res=>res.json()).then(data=>{ if(data.success) { alert('Профиль обновлён! Страница перезагрузится.'); location.reload(); } else alert('Ошибка: '+data.error); });
    });
    
    function loadFriendRequests() {
        fetch('/get_friend_requests').then(r=>r.json()).then(data=>{
            const container = document.getElementById('friendRequestsList');
            if(data.requests && data.requests.length) {
                container.innerHTML = data.requests.map(r => `<div style="display:flex;justify-content:space-between;margin:5px 0;"><span>📨 ${escapeHtml(r)}</span><button onclick="acceptFriend('${escapeHtml(r)}')" style="background:#10b981;border:none;border-radius:16px;padding:2px 12px;color:white;">Принять</button></div>`).join('');
            } else container.innerHTML = '<p>Нет заявок</p>';
        });
    }
    ...
    window.acceptFriend = (name) => {
        fetch('/accept_friend', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({friend:name})})
            .then(r=>r.json()).then(data=>{ alert(data.message); loadFriendRequests(); });
    };
    
    function addMessage(name, text, time, isOwn, avatar, avatarBase64) {
        let div = document.createElement('div');
        div.className = `message ${isOwn ? 'message-own' : ''}`;
        let avatarHtml = avatarBase64 ? `<img src="${avatarBase64}" style="width:100%;height:100%;">` : (avatar || '👤');
        div.innerHTML = `<div class="message-avatar" onclick="showUserProfile('${escapeHtml(name)}')">${avatarHtml}</div>
            <div class="message-content"><div class="message-name">${escapeHtml(name)}<span class="message-time">${time}</span></div>
            <div class="message-text">${escapeHtml(text)}</div></div>`;
    ...
    socket.on('history', (history) => { messagesDiv.innerHTML = ''; history.forEach(msg => addMessage(msg.name, msg.text, msg.time, msg.name === username, msg.avatar, msg.avatar_base64)); });
    socket.on('new_message', (msg) => addMessage(msg.name, msg.text, msg.time, msg.name === username, msg.avatar, msg.avatar_base64));
    ...
    socket.on('rooms_update', (roomsList) => {
        ...
        document.querySelectorAll('.room-item').forEach(el => { el.onclick = () => {
            ...
            currentRoom = newRoom;
            document.getElementById('currentRoomSpan').innerText = currentRoom;
    ...
    socket.on('users_update', (usersList) => {
        ...
        container.innerHTML = usersList.map(u => `<div class="user-item" onclick="showUserProfile('${escapeHtml(u.name)}')"><span class="online-dot"></span> ${u.avatar_base64 ? `<img src="${u.avatar_base64}" style="width:20px;height:20px;border-radius:50%;">` : (u.avatar || '👤')} ${escapeHtml(u.name)} ${u.role === 'owner' ? '<span class="badge-owner">ВЛ</span>' : (u.role === 'admin' ? '<span class="badge-admin">АДМ</span>' : '')}</div>`).join('');
    ...
    socket.on('friends_update', (friendsList) => {
        ...
        if(!container) return;
        container.innerHTML = friendsList.map(f => `<div class="user-item" onclick="showUserProfile('${escapeHtml(f.name)}')">👫 ${escapeHtml(f.name
