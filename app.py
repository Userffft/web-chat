...
function showToast(message, type='success'){
    ...
    toast.style.background=type==='success'?'#10b981':(type==='error'?'#ef4444':'#4f46e5');
...
document.getElementById('changeIdBtn').onclick=()=>{
    ...
    if(newId && /^\\d{4,8}$/.test(newId)){
        fetch('/change_id',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({new_id:newId})}).then(r=>r.json()).then(data=>{
            if(data.success){showToast('✅ ID успешно изменён!','success');setTimeout(()=>location.reload(),1500);}
...
window.copyToClipboard=(text)=>{navigator.clipboard.writeText(text);showToast('✅ ID скопирован в буфер обмена','success');};
window.muteUser=(name)=>{let time=document.getElementById('muteTimeSelect').value;fetch('/mute_user',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:name,minutes:time})}).then(r=>r.json()).then(data=>{addSystem(data.message);showToast(data.message,'warning');document.getElementById('userModal').style.display='none';});};
...
window.addFriend=name=>{fetch('/add_friend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({friend:name})}).then(r=>r.json()).then(data=>{addSystem(data.message);showToast(data.message,'success');addNotification('Заявка отправлена',name);document.getElementById('userModal').style.display='none';});};
...
window.acceptReq=name=>{fetch('/accept_friend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({friend:name})}).then(r=>r.json()).then(data=>{addSystem(data.message);showToast(data.message,'success');addNotification('Новый друг',name);pendingFriendRequests=pendingFriendRequests.filter(r=>r!==name);updateNotifyBadge();updateNotificationsList();loadRequests();});};
