...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui;background:linear-gradient(135deg,#667eea,#764ba2);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}.card{background:rgba(255,255,255,0.95);backdrop-filter:blur(10px);border-radius:48px;padding:48px;max-width:400px;width:100%;box-shadow:0 25px 50px -12px rgba(0,0,0,0.25)}h1{text-align:center;margin-bottom:32px;background:linear-gradient(135deg,#667eea,#764ba2);-webkit-background-clip:text;-webkit-text-fill-color:transparent}input{width:100%;padding:16px;border:2px solid #e5e7eb;border-radius:32px;margin-bottom:16px;font-size:16px;outline:none}input:focus{border-color:#667eea;box-shadow:0 0 0 3px rgba(102,126,234,0.1)}button{width:100%;padding:16px;background:linear-gradient(135deg,#667eea,#764ba2);color:white;border:none;border-radius:32px;font-size:16px;font-weight:600;cursor:pointer}button:hover{transform:translateY(-2px)}.error{background:#fee2e2;color:#dc2626;padding:12px;border-radius:24px;margin-bottom:20px;text-align:center}.footer{text-align:center;margin-top:24px;color:#6b7280}a{color:#667eea;text-decoration:none}
</style>
</head>
...
body{font-family:system-ui,'Segoe UI';background:linear-gradient(135deg,#1e1b4b,#4c1d95);height:100vh;display:flex;transition:0.3s}
body.dark{background:#0f172a}
.sidebar{width:300px;background:rgba(255,255,255,0.95);backdrop-filter:blur(10px);border-right:1px solid rgba(0,0,0,0.1);display:flex;flex-direction:column;overflow-y:auto}
body.dark .sidebar{background:#1e293b;color:#fff;border-color:#334155}
.user-card{text-align:center;padding:24px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;cursor:pointer;margin-bottom:16px}
.user-avatar{font-size:64px;margin-bottom:12px}
.user-name{font-size:1.2rem;font-weight:700}
.user-bio{font-size:12px;opacity:0.85;margin-top:5px}
.section-title{font-weight:600;padding:16px 20px 8px 20px;color:#475569;font-size:13px;text-transform:uppercase;letter-spacing:0.5px}
...
.add-room{display:flex;gap:8px;margin:12px 12px 0 12px}
.add-room input{flex:1;padding:10px 16px;border:1px solid #e2e8f0;border-radius:40px;outline:none;background:#fff}
body.dark .add-room input{background:#334155;border-color:#475569;color:#fff}
.add-room button{background:#4f46e5;color:#fff;border:none;border-radius:40px;padding:8px 20px;cursor:pointer}
.chat-area{flex:1;display:flex;flex-direction:column;background:#fff}
body.dark .chat-area{background:#0f172a}
.chat-header{padding:20px 24px;background:#fff;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center}
body.dark .chat-header{background:#1e293b;border-color:#334155;color:#fff}
...
.message-avatar{width:40px;height:40px;background:linear-gradient(135deg,#667eea,#764ba2);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;cursor:pointer}
.message-content{background:#f1f5f9;padding:10px 16px;border-radius:20px;max-width:65%;border-bottom-left-radius:4px}
body.dark .message-content{background:#334155;color:#fff}
.message-own .message-content{background:#4f46e5;color:#fff;border-bottom-right-radius:4px;border-bottom-left-radius:20px}
...
.message-text{word-wrap:break-word;margin-top:4px;font-size:14px}
.badge-owner{background:#ef4444;font-size:9px;padding:2px 8px;border-radius:20px}
.badge-admin{background:#10b981;font-size:9px;padding:2px 8px;border-radius:20px}
.online-dot{width:10px;height:10px;background:#10b981;border-radius:50%;display:inline-block;margin-right:8px}
.system-msg{text-align:center;font-size:12px;color:#64748b;padding:8px;margin:8px 0}
.typing-indicator{padding:8px 24px;font-size:12px;color:#64748b;font-style:italic}
.input-area{display:flex;gap:12px;padding:20px 24px;background:#fff;border-top:1px solid #e2e8f0}
body.dark .input-area{background:#1e293b;border-color:#334155}
.input-area input{flex:1;padding:14px 20px;border:2px solid #e2e8f0;border-radius:40px;outline:none;font-size:14px}
body.dark .input-area input{background:#334155;border-color:#475569;color:#fff}
.input-area button{background:#4f46e5;border:none;border-radius:50%;width:48px;height:48px;color:#fff;cursor:pointer;font-size:18px;transition:transform 0.2s}
.input-area button:hover{transform:scale(1.05)}
.btn-settings,.btn-logout{margin:8px 12px;padding:12px;border-radius:20px;cursor:pointer;font-weight:500;border:none}
...
.modal-content{background:#fff;border-radius:32px;padding:32px;max-width:400px;width:90%;max-height:80vh;overflow-y:auto}
body.dark .modal-content{background:#1e293b;color:#fff}
.modal-content input,.modal-content textarea{width:100%;padding:12px;margin:10px 0;border:1px solid #e2e8f0;border-radius:24px;outline:none}
body.dark .modal-content input,body.dark .modal-content textarea{background:#334155;border-color:#475569;color:#fff}
.modal-content button{background:#4f46e5;color:#fff;border:none;padding:12px;border-radius:24px;width:100%;cursor:pointer;margin-top:10px}
.close{float:right;font-size:28px;cursor:pointer;line-height:20px}
.user-menu{position:fixed;background:#fff;border-radius:16px;box-shadow:0 10px 25px rgba(0,0,0,0.1);padding:8px;z-index:1000;display:none;min-width:160px}
body.dark .user-menu{background:#1e293b;box-shadow:0 10px 25px rgba(0,0,0,0.3)}
.user-menu button{width:100%;padding:10px 16px;border:none;background:none;text-align:left;cursor:pointer;border-radius:12px;font-size:14px}
.user-menu button:hover{background:#f1f5f9}
body.dark .user-menu button:hover{background:#334155;color:#fff}
.emoji-picker{position:absolute;bottom:80px;left:20px;background:#fff;border-radius:20px;padding:12px;display:none;grid-template-columns:repeat(6,1fr);gap:8px;box-shadow:0 10px 25px rgba(0,0,0,0.1);z-index:1000}
body.dark .emoji-picker{background:#1e293b}
.emoji{font-size:24px;cursor:pointer;text-align:center;padding:5px;border-radius:12px}
.emoji:hover{background:#f1f5f9}
body.dark .emoji:hover{background:#334155}
...
document.querySelectorAll('.emoji').forEach(el => {
    el.onclick = () => { msgInput.value += el.textContent; msgInput.focus(); picker.style.display = 'none'; };
});
document.getElementById('emojiBtn').onclick = () => {
    picker.style.display = picker.style.display === 'grid' ? 'none' : 'grid';
};
document.addEventListener('click', (e) => {
    if(!e.target.closest('#emojiBtn') && !e.target.closest('.emoji-picker')) picker.style.display = 'none';
    if(!e.target.closest('.user-item') && !e.target.closest('.user-menu')) document.getElementById('userMenu').style.display = 'none';
});
...
function showUserMenu(name, x, y) {
    ...
    fetch('/user_info/' + encodeURIComponent(name)).then(r=>r.json()).then(data => {
        ...
        setTimeout(() => { if(currentMenuUser === name) { menu.style.display = 'none'; currentMenuUser = null; } }, 5000);
    });
}
...
window.addFriend = function(name) {
    fetch('/add_friend', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({friend:name})})
        .then(r=>r.json()).then(data=>{addSystemMessage(data.message); document.getElementById('userMenu').style.display = 'none';});
};
...
function addSystemMessage(text) {
    let div = document.createElement('div'); div.className = 'system-msg'; div.textContent = text; msgDiv.appendChild(div); msgDiv.scrollTop = msgDiv.scrollHeight;
}
...
socket.on('users', (list) => {
    ...
    container.innerHTML = list.map(u => `<div class="user-item" data-user="${escape(u.name)}" oncontextmenu="event.preventDefault();showUserMenu('${escape(u.name)}', event.clientX, event.clientY);" onclick="showUserMenu('${escape(u.name)}', event.clientX, event.clientY)"><span class="online-dot"></span> ${u.avatar || '👤'} ${escape(u.name)} ${u.role === 'owner' ? '<span class="badge-owner">ВЛ</span>' : (u.role === 'admin' ? '<span class="badge-admin">АДМ</span>' : '')}</div>`).join('');
...
socket.on('friends', (list) => {
    ...
    if(container) container.innerHTML = list.map(f => `<div class="user-item" data-user="${escape(f.name)}" onclick="viewProfile('${escape(f.name)}')">👫 ${escape(f.name)}</div>`).join('');
});
...
document.getElementById('settingsBtn').onclick = () => { settingsModal.style.display = 'flex'; loadRequests(); };
...
document.getElementById('saveProfile').onclick = () => {
    ...
    })}).then(r=>r.json()).then(data => { if(data.success) { alert('Сохранено! Страница перезагрузится.'); location.reload(); } else alert('Ошибка: ' + data.error); });
};
